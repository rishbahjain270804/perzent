...
export async function runMaintenance(): Promise<MaintenanceSummary> {
  ...
  // Time-boxed to fit the 60 s function budget; a leftover backlog carries over to the next run and
  // purge_retention never deletes un-archived points before a grace period.
  const compaction = await compactClosedShifts({ minAgeHours: 3, limit: 1000, timeBudgetMs: 40_000 });
